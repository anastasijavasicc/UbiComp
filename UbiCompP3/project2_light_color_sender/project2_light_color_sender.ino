...
unsigned long lightAlertUntil = 0;
unsigned long manualActuatorUntil = 0;
...
void handleSerialCommands() {
  while (Serial.available()) {
    ...
    if (command == "MONITORING_ON") {
      active = true;
      digitalWrite(LED_PIN, HIGH);
      Serial.println("{\"event\":\"mcu_command\",\"command\":\"monitoring_on\",\"active\":1}");
    }
    else if (command == "MONITORING_OFF") {
      active = false;
      digitalWrite(LED_PIN, LOW);
      Serial.println("{\"event\":\"mcu_command\",\"command\":\"monitoring_off\",\"active\":0}");
    }
    else if (command == "ACTUATOR_SIMULATE") {
      manualActuatorUntil = millis() + 2000;
      Serial.println("{\"event\":\"mcu_command\",\"command\":\"simulate_actuator\",\"status\":\"triggered\"}");
    }
  }
}

void updateLedState() {
  if (manualActuatorUntil > millis()) {
    setRgbRed();
  }
  else if (lightAlertUntil > millis()) {
    setRgbRed();
  }
  else {
    if (active) {
